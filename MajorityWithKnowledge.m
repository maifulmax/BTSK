...
%         err('reach to convergence')
    ...
    for task_j=1:Ntask
        ...
        count(:,2)=count_new(:,2)/sum(count_new(:,2));
        for k = 1:size(count,1)
            P_r(task_j, count(k, 1)) = count(k,2);
        end
        count=sortrows(count,[1 -2]);
...
if ~isempty(model.true_labels)
    size_ans_labels = size(ans_labels)
    size_true_labels = size(model.true_labels)
